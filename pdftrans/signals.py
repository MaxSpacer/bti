...
def export_data_pdf(sender, instance, created, **kwargs):
    ...
    # print(created)
        ...
        if instance.doc_type == 'Поэтажный план и экспликация на помещение' and on == 'on':
        ...
        #         explication_adress_areas=['50,680,780,100']
            ...
            for page in doc:
                if page.number > 0:
                    ...
                    print('floor_list2')
                    print(floor_list)
                    # create_img = get_or_create(defaults__exact='bar', defaults={'defaults': 'baz'})
                    obj, created = OrderImage.objects.update_or_create(
                                                                        order_fk = instance,
                                                                        floor_for_image = floor_item,
                                                                        order_image = path_img_scheme_bd,
                                                                        defaults={'order_fk': instance, 'order_image': path_img_scheme_bd}
                                                                      )
        ...
        print('instance.get_order_adress')
        print(instance.get_order_adress())
        print(instance.get_order_adress().get_full_adress())
